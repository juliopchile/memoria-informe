...
const translations = {
  ...
  ar: {
    ...
    abstract2:
      "يقوم هذا المشروع بتنقيح قاعدة بيانات لتجزئة الحالات لأسماك السلمون، بهدف تدريب نماذج YOLO بأداء متفوق. بالإضافة إلى ذلك، تُحسَّن المعاملات الفائقة أثناء التدريب وتُصدَّر النماذج إلى TensorRT لتقليل أزمنة الاستدلال.",
    abstract3:
      "تُظهر النتائج أن قاعدة بيانات أكثر دقة يمكن أن تعزّز الأداء في مهام التتبع عبر تحقيق مقاييس ارتباط زمني أفضل (MOTA، HOTA، IDFW) وتحسين جودة التجزئات؛ وأن تحسين المعاملات الفائقة يحقق نتائج أفضل؛ وأن التحويل إلى TensorRT يقلّل بصورة ملحوظة أزمنة الاستدلال مع خسارة أداء طفيفة.",
    linksTitle: "روابط",
  ...
  fr: {
    ...
    abstract3:
      "Les résultats montrent qu’une base de données plus précise peut améliorer les performances dans les tâches de suivi en obtenant de meilleures métriques d’association temporelle (MOTA, HOTA, IDFW) et en améliorant la qualité des segmentations; que l’optimisation des hyperparamètres produit de meilleurs résultats; et que la conversion vers TensorRT réduit significativement les temps d’inférence avec une perte de performance minimale.",
    linksTitle: "Liens",
  ...
  he: {
    ...
    abstract3:
      "התוצאות מראות שמסד נתונים מדויק יותר יכול לשפר את הביצועים במשימות מעקב באמצעות השגת מדדי שיוך זמני טובים יותר (MOTA, HOTA, IDFW) ולשפר את איכות הסגמנטציות; שאופטימיזציית ההיפר־פרמטרים מניבה תוצאות טובות יותר; והמרה ל‑TensorRT מקטינה משמעותית את זמני האינפרנס עם אובדן ביצועים מזערי.",
    linksTitle: "קישורים",
  ...
  ja: {
    ...
    abstract2:
      "本プロジェクトでは、サケのインスタンスセグメンテーション用データベースを洗練し、より高性能なYOLOモデルの学習を目指しました。さらに、学習においてハイパーパラメータを最適化し、推論時間短縮のためにモデルをTensorRTへエクスポートしています。",
    abstract3:
      "結果は、より精緻なデータベースが、時間的対応指標（MOTA、HOTA、IDFW）の向上を通じて追跡タスクの性能を高め、セグメンテーション品質を改善し得ること、ハイパーパラメータ最適化がより良い成果をもたらすこと、そしてTensorRTへの変換が最小限の性能低下で推論時間を大幅に短縮することを示しています。",
    linksTitle: "リンク",
    pdf: "PDFを読む（スペイン語のみ）",
    code: "プロジェクトコード",
    latex: "LaTeXソース",
  ...
  ru: {
    ...
    abstract2:
      "В проекте уточняется база данных сегментации экземпляров лосося с целью обучения моделей YOLO с превосходной производительностью. Кроме того, в процессе обучения оптимизируются гиперпараметры, а модели экспортируются в TensorRT для сокращения времени инференса.",
    abstract3:
      "Результаты показывают, что более точная база данных может повысить производительность в задачах отслеживания, достигая лучших метрик временной ассоциации (MOTA, HOTA, IDFW), а также улучшить качество сегментаций; оптимизация гиперпараметров даёт лучшие результаты; а преобразование в TensorRT существенно снижает время инференса при минимальной потере производительности.",
    linksTitle: "Ссылки",
  ...
  uk: {
    ...
    abstract2:
      "У цьому проєкті уточнюють базу даних сегментації інстанцій лосося з метою навчання моделей YOLO із вищою продуктивністю. Крім того, під час навчання оптимізують гіперпараметри, а моделі експортують до TensorRT, щоб скоротити час інференсу.",
    abstract3:
      "Результати демонструють, що точніша база даних може підвищити продуктивність у задачах відстеження, досягаючи кращих метрик часової асоціації (MOTA, HOTA, IDFW), і поліпшити якість сегментацій; оптимізація гіперпараметрів дає кращі результати; а перетворення в TensorRT істотно зменшує час інференсу за мінімальних втрат продуктивності.",
