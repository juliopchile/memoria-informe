\part*{Capítulo 2: Estado del Arte}\addcontentsline{toc}{part}{Capítulo 2: Estado del Arte}
% TODO Corrección [7] [DONE]: Reestructurar: Este archivo pasará a ser Capítulo 2 Estado del Arte con secciones 2.1 Soluciones existentes (contenido actual de esta subsección) y 2.2 Herramientas y Métodos (mover parte técnica al archivo 8_arte.tex). Añadir 2.3 Discusión (comparar solución propuesta versus existentes; destacar contribuciones: dataset depurado, marco conjunto segmentación+tracking, integración optimización TensorRT).
\section{Soluciones Existentes}
% TODO lenguaje: “acuícola” con tilde; revisar “actividad acuicola” en el párrafo siguiente.
% TODO Corrección [8] [DONE]: En este segundo párrafo añadir explícitamente relevancia de estimación de biomasa (ligada a precisión volumétrica y salud) y mención de reducción de remuestreo gracias a mejor tracking.
...
La acuicultura constituye cerca del 60\% de la producción de productos marinos en el mundo \cite{FAO2024} y la piscicultura forma parte importante de la actividad acuícola en Chile \cite{chile2017levantamiento}. Es entonces de vital importancia el monitoreo de la salud de los peces (estimación de talla, biomasa y lesiones) por medio de revisiones regulares para asegurar una alimentación adecuada y la detección temprana de enfermedades. Por desgracia, las mediciones suelen depender de las condiciones meteorológicas, exigen mucho tiempo y trabajo, además de ser invasivas y estresantes para los peces \cite{Tonachella2022} (Fig. \ref{fig:medicion_tradicional_peces}).
% TODO lenguaje: Evitar doble conector “además de”; alternativa: “y también son invasivas...”.

\TwoImgs[0.9\textwidth][1em]
{figures/midiendo_pez.jpg}% Image_1
{Midiendo el pez.\label{fig:midiendo_pez}}% Caption_1 & Label_1 (optional)
{figures/maquina_para_pesar_peces.png}% Image_2
{Máquina para pesar peces.\label{fig:maquina_para_pesar_peces}}% Caption_2 & Label_2 (optional)
[Técnicas tradicionales de medición de talla y peso en peces]% Global short caption
{Técnicas tradicionales de medición de talla y peso en peces.}% Global caption
{fig:medicion_tradicional_peces}% Global label
    
En este contexto, la visión por computadora resulta especialmente relevante para la estimación precisa de biomasa, talla y caracterización de salud, ya que permite calcular el volumen y peso de los peces de manera no invasiva, lo que está directamente ligado a la salud y productividad del cultivo a la vez que son más escalables y menos costosas.
% TODO lenguaje: Concordancia: “a la vez que resultan más escalables y menos costosas”.

Ejemplos de solución propuestas incluyen métodos de estimación estadística para la talla de peces mediante sensores, radares y cámaras \cite{Tonachella2022} (Fig. \ref{fig:workflow_tonachella}), y el uso de modelos de aprendizaje automático en detección de enfermedades \cite{Ahmed2022}. %Estas soluciones prometen ser más escalables, menos costosas y menos intrusivas en el ciclo de vida de los animales.
...
% TODO Corrección [8] [DONE]: Reemplazar “modelos tridimensionales generados en Blender, para la estimación de volumen y masa en peces” por “modelos tridimensionales para la estimación de volumen y masa en peces” (ya no se usa Blender por desempeño). No modificado automáticamente.
% TODO Corrección [8] [DONE]: Evaluar inclusión de técnicas adicionales usadas en acuicultura: monitoreo de lesiones con redes de clasificación, estimación automática de comportamiento (schooling patterns), análisis de alimentación mediante detección de pellets, sensores multiespectrales para salud, integración sonar + visión.

WildSense por su parte implementa una novedosa técnica basada en el emparejamiento de mapas de profundidad obtenidos con cámaras estéreo y modelos tridimensionales para la estimación de volumen y masa en peces. Una explicación más detallada de su flujo de trabajo se encuentra en la sección \ref{sec:workflow_wildsense}.

Además, entre las proyecciones futuras de WildSense se encuentra la detección y caracterización de nivel de progresión de enfermedades; inspección de mallas en jaulas de cultivo; y monitoreo subacuático (temperatura, oxígeno saturado, sólidos disueltos, pH) \cite{wildsense-2}.
% TODO lenguaje: Coma tras el conector: “Además,”.


\section{Herramientas y Métodos}
% TODO lenguaje: Uniformar “a través” con tilde; “hiperparámetros” con tilde en todo el capítulo.
% TODO Corrección [7] [DONE]: Este archivo debe integrarse como sección 2.2 Herramientas y Métodos (actual "Estado del arte" pasará a ser parte del Capítulo 2 reestructurado). Incluir encabezado adecuado y ajustar numeración tras mover 7_MarcoTeorico a 2.1.
% TODO Corrección [7] [DONE]: Al final de este capítulo agregar subsección 2.3 Discusión (contrastar solución propuesta vs. soluciones existentes; justificar elección de tracking-by-detection con métricas HOTA, AssA, IDF1, MOTA; destacar contribución de depuración de dataset y exportación eficiente).
...
% TODO lenguaje: “píxeles” con tilde; “imágenes” con tilde.
...
\label{sec:modelos_segmentación}
Diversas arquitecturas han sido desarrolladas para abordar el problema de la segmentación de instancias, con especial relevancia en aplicaciones de tiempo real. Para una exploración más detallada sobre la evolución de los modelos de segmentación de instancias, puede consultarse la sección \ref{sec:anexo_sota_segmentacion} en el anexo. Se describen a continuación algunos de los modelos más representativos para este propósito.
...
MS-RCNN \cite{MaskScoringRCNN} representa una mejora sobre Mask R-CNN mediante la incorporación del mecanismo denominado ``\textit{mask scoring}'', que mide la precisión de las máscaras segmentadas para cada objeto en vez de asumir una confianza fija. Esto se logra introduciendo una capa adicional que evalúa la calidad de cada máscara generada, mejorando así el rendimiento en la segmentación de objetos y reduciendo las falsas detecciones.
...
La versión YOLOv9 \cite{YOLOv9} introduce innovaciones encaminadas a mejorar la precisión y la eficiencia, haciendo uso de mecanismos como \textit{Programmable Gradient Information} (PGI), que optimiza la transmisión de gradientes mediante una rama reversible auxiliar. Además, se incorporan \textit{Generalized Efficient Layer Aggregation Network} (GELAN), que facilita una mejor utilización de los parámetros y posibilita configuraciones ajustables según el dispositivo.
...
% TODO Corrección [8] [DONE]: Corregir redacción "reducción en consumo energéticos" -> "reducción en consumo energético" en el párrafo siguiente.
...
% TODO lenguaje: “interés”, “video/video” unificar; “detecciones” con tilde.
% TODO Corrección [8] [DONE]: En la subsección 3.5.1 (equivalente tras reordenar) enfatizar explícitamente que dada la utilización de modelos de detección y segmentación de instancia, tracking-by-detection es el enfoque adecuado (agregar párrafo de enlace con objetivos y validación).
En su forma más simple, el seguimiento puede definirse como el problema de estimar la trayectoria de un objeto en el plano de la imagen a medida que se desplaza por una escena. En otras palabras, un \textit{tracker} asigna etiquetas coherentes a los objetos rastreados en diferentes fotogramas de un video. Además, dependiendo del dominio de seguimiento, un \textit{tracker} también puede proporcionar información centrada en el objeto, como la orientación, el área o la forma de un objeto \cite{Yilmaz2006}.
...
Otra forma de categorizar la tarea de seguimiento es según su implementación basada o no en detecciones previas. En el marco de esta memoria, dada la utilización de modelos de detección y segmentación de instancia, resulta adecuada la especial consideración en técnicas de seguimiento basado en detección.
...
\begin{itemize}
    ...
    \item \textbf{Seguimiento basado en modelos}: utiliza un modelo predefinido del objeto, el cual puede ser predeterminado o aprendido, simple (forma, color, etc.) o complejo (representación semántica o basada en contexto) y lo ajusta en cada fotograma. En resumen se realiza detección al mismo tiempo que se sigue al objeto.
\end{itemize}

\subsubsection{Algoritmos de seguimiento relevantes}
Entre los algoritmos desarrollados para el seguimiento de objetos destacan los siguientes:
\begin{itemize}
...
\section{Discusión sobre el Estado del Arte}
% TODO lenguaje: “heterogéneos” con tilde; “evalúan” con tilde; “dataset” en cursiva.
La solución implementada por WildSense entrena modelos sobre bases de datos heterogéneos sin filtrar y evalúan segmentación sin seguimiento. La propuesta de este trabajo se diferencia al poner primero la depuración programática del \textit{dataset} para eliminar instancias de bajo valor y se introduce además un marco integrado de validación que alinea métricas espaciales (precisión, recall, F1, mAP@50, mAP@50-95) con métricas temporales y de asociación (HOTA, AssA, IDF1, MOTA, MT\%, IDSW, Frag) para medir impacto conjunto sobre segmentación y seguimiento.

Por otro lado se prefiere evaluar variantes YOLO (v8, v9, 11) pues equilibran precisión y velocidad a diferencia de arquitecturas más pesadas (Mask-/MS-RCNN, SOLOv2) que superan en rendimiento pero penalizan el \textit{throughput} al ser más lentas. Además YOLO permite de forma nativa (a través de Ultralytics) exportar a TensorRT para optimizar latencia; entrenamiento con búsqueda de hiperparámetros; y seguimiento por detección (ByteTrack, BoT-SORT).
% TODO lenguaje: “a través” con tilde; considerar traducir “throughput” por “rendimiento” o “tasa de procesamiento”.
