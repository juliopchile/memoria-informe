...
En primera instancia se tenía la idea de crear un filtro capaz de utilizar datos de profundidad asociadas a las imágenes de la base de datos con el fin de filtrarlas, en caso de encontrarse a una distancia mayor a la de un umbral dado. Sin embargo, al no contar con datos de profundidad asociados a las imagenes del \textit{dataset}, se optó por un filtro más sencillo que es el utilizado.

Si bien no se utilizó esta versión del filtro, si estaba planeada su inclusión como material complementario de este proyecto. Lamentablemente el código se perdió en un formateo de disco duro y no fue posible su recuperación. Sin embargo, se incluye una imagen que muestra el funcionamiento del filtro de profundidad como evidencia de su existencia (Fig. \ref{fig:filtro_profundiad}).

\begin{figure}[htbp]
\begin{center}
    \includegraphics[width=0.8\textwidth]{figures/filtro_de_profunidad.png}
    \caption[Implementación del filtro de profundidad]{Implementación del filtro de profundiad. Como puede verse en la máscara binaria de la derecha, algunas instancias se encuentran fuera del umbral de distancia (en este caso 3 metros) y por ende son excluidos del \textit{dataset}.}
