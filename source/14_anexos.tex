...
En primera instancia se tenía la idea de crear un filtro capaz de utilizar datos de profundidad asociadas a las imágenes de la base de datos con el fin de filtrarlas, en caso de encontrarse a una distancia mayor a la de un umbral dado. Sin embargo, al no contar con datos de profundidad asociados a las imagenes del \textit{dataset}, se optó por un filtro más sencillo que es el mostrado más arriba.

Si bien no se utilizó este filtro, si estaba planeada su inclusión como material complementario de este proyecto. Lamentablemente el código se perdió en un formateo de disco duro y no se pudo recuperar. Sin embargo, se incluye una imagen que muestra el funcionamiento del filtro de profundidad como evidencia de su existencia (Fig. \ref{fig:filtro_profundiad}).
